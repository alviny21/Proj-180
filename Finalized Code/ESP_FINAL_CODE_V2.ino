  ...
  switch (systemState) {
    ...
    case COLLECTING: {
      ...
      if (currentTime - lastSampleTime >= sampleInterval) {
        ...
        // === Read BendLab Sensor Data by Address (0x12–0x15) ===
        const uint8_t expected_addrs[4] = {0x12, 0x13, 0x14, 0x15};
        for (int i = 0; i < 4; i++) {
          bool found = false;
          for (int j = 0; j < device_count; j++) {
            if (addresses[j] == expected_addrs[i]) {
              ads_hal_select_device(j);
        ...
        // === Read IMU Data if address 0x28 is present ===
        ...
        if (imu_found) {
          ...
          output += String(angVelData.gyro.z, 2);
        } else {
