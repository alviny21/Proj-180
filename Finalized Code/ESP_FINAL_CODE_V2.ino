...
void setup() {
  ...
  setLEDColor(255, 255, 255);  // White = power on
  ...
  // === SD Card Setup ===
  if (!SD.begin(SD_CS)) {
    Serial.println("SD Initialization failed: Card not detected.");
  ...
  if (!dataFile) {
    Serial.println("SD Initialization failed: Could not create log file.");
    setLEDColor(255, 0, 0); // Solid red
    while (true); // Halt
  }
  ...
  if (ret_val == ADS_OK) {
    ...
    Serial.println("BendLab initialization succeeded.");
...
void loop() {
  ...
  switch (systemState) {
    ...
    case COLLECTING: {
      ...
      if (ledState)
              pixel.setPixelColor(0, readyBlinkColor);
            else
